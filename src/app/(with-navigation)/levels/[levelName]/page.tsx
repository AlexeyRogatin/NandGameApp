...
export default function Level({ params }: { params: { levelName: string } }) {
  const router = useRouter();

  const levelName = decodeURI(params.levelName);
  ...
  const handleCheck = () => {
    axios
      .post("/api/solve", scheme.pack(level!))
      .then((response) => {
        if (response.data.success) {
          setResultOpen(true);
          setResults({
            components: response.data.componentCount,
            nands: response.data.nandCount,
          });
        } else {
          setFailsOpen(true);
    ...
    <>
      <div className="flex flex-1 gap-3">
        <div className="flex flex-col w-1/6 justify-between flex-wrap">
          <div className="flex flex-col flex overflow-y gap-3">
            <h2>{level?.name}</h2>
            ...
            {level.tests.show && (
              <table>
                <tbody>
                  ...
                  <tr>
                    {level?.tests.inArgs.map((name, index) => (
                      <th key={index}>{name}</th>
                    ))}
                    {level?.tests.outArgs.map((name, index) => (
                      <th key={index}>{name}</th>
                    ))}
                  </tr>
                  {level?.tests.data.map((test, index) => {
                    return (
                      test.show && (
                        <tr key={index}>
                          {test.inData.map((v, index) => (
                            <td key={index}>{v}</td>
                          ))}
                          {test.outData.map((v, index) => (
                            <td key={index}>{v}</td>
                          ))}
                        </tr>
                      )
          ...
          <div className="w-full">
            <button onClick={handleCheck} className="bordered w-full">
              Check solution
            </button>
          </div>
        </div>
      ...
      <Dialog
        ...
        onClose={() => {
          ...
          if (levelIndex + 1 !== levels.length) {
            router.push("/levels/" + levels[levelIndex + 1].name);
          } else {
            router.push("/levels");
          }
        }}
      >
        <p>
          Your solution is correct. It has {results.components} components{" "}
          {results.nands !== 0 && <>and {results.nands} Nand elements.</>}
        </p>
      </Dialog>
        ...
        <table>
          <tbody>
            ...
            {fails.slice(0, 10).map((fail, index) => {
              ...
              return (
                <tr key={index}>
                  {test.inData.map((data, index) => (
                    <td key={index}>{data}</td>
                  ))}
                  {test.outData.map((data, index) => (
                    <td key={index}>{data}</td>
                  ))}
                  {fail.outArgs.map((data, index) => (
                    <td key={index}>{data}</td>
                  ))}
                </tr>
              );
